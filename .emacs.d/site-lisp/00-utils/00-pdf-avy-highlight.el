;;; 00-pdf-avy-highlight.el --- Highlight in pdf-tools with keyboard  -*- lexical-binding: t; -*-
...
(defun pdf-keyboard-highlight ()
  ...
  (let* ((start (get-coordinates nil))
	 ...
	 (type (intern (completing-read "Markup type (default highlight): "
                                      '("squiggly" "highlight" "underline" "strike-out")
                                      nil t)))
	 (color (pdf-annot-read-color)))
    (pdf-annot-add-markup-annotation
     edges type color) nil))

(provide '00-pdf-avy-highlight)
